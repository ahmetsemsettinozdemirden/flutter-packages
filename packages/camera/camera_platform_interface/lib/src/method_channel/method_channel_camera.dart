...
class MethodChannelCamera extends CameraPlatform {
  ...
  MethodChannelCamera() {
    const MethodChannel channel = MethodChannel('flutter.io/cameraPlugin/device');
  ...
  Stream<CameraEvent> _cameraEvents(int cameraId) => cameraEventStreamController.stream.where((CameraEvent event) => event.cameraId == cameraId);
  ...
  Future<int> createCamera(
    ...
    bool enableAudio = false,
      ...
      createCameraWithSettings(cameraDescription, MediaSettings(resolutionPreset: resolutionPreset, enableAudio: enableAudio));
  ...
  Future<int> createCameraWithSettings(
    ...
    try {
      ...
      final Map<String, dynamic>? reply = await _channel.invokeMapMethod<String, dynamic>('create', <String, dynamic>{
        ...
        'resolutionPreset': resolutionPreset != null ? _serializeResolutionPreset(mediaSettings.resolutionPreset!) : null,
  ...
  Future<void> initializeCamera(
    ...
    _channels.putIfAbsent(cameraId, () {
      final MethodChannel channel = MethodChannel('flutter.io/cameraPlugin/camera$cameraId');
  ...
  @override
  Future<void> setFocusDistance(int cameraId, double focusDistance) async {
    try {
      await _channel.invokeMethod<double>(
        'setFocusDistance',
        <String, dynamic>{
  ...
  Future<dynamic> handleDeviceMethodCall(MethodCall call) async {
    switch (call.method) {
      case 'orientation_changed':
        ...
        deviceEventStreamController.add(DeviceOrientationChangedEvent(deserializeDeviceOrientation(arguments['orientation']! as String)));
      default:
